...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "unsolved_set" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("unsolved_set")
        ...
        metric_dict = {}
        metric_dict["solve_f1"] = np.mean(reward_extra_infos_dict["solve_f1"])
        metric_dict["solve_rate"] = np.mean(reward_extra_infos_dict["solve_rate"])
        metric_dict["solve_precision"] = np.mean(reward_extra_infos_dict["solve_precision"])
        print(metric_dict)
        return metric_dict
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "codes" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("codes")
                if "unsolved_set" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("unsolved_set")
                gen_batch = batch.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
